 ...
 #   end
  ...
  def get_data 
    ...
    data[0].each do |item|
      ...
      arr << ["#{item.father.code}[#{item.father.strain.common_name}]", "#{item.mother.code}[#{item.mother.strain.common_name}]","#{Framework.all.index(item.basket.framework)+1}-#{item.basket.code}", item.cage_at, item.show_can_usable.html_safe, item.breeding, item.creator.full_name, op_str]
    end
